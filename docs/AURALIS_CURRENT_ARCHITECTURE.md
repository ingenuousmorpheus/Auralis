...
- **Session 017 additions:**
  ...
  - **Played chords:** the masked CQT first goes through `fundamentals`:
    - Only pitch peaks are kept, because a note leaks into the semitones either side of it.
    - Each lower note's harmonic series is explained away (`OVERTONE_DECAY` 0.6/k).
    - Without this, a D triad shows C♯ (leakage plus F♯'s third harmonic) and reads as Dmaj7.
  - `played_chords` then does per-bar template matching on that chroma (bass counts more):
    - Triads, plus `maj7`, dominant `7` and minor `7`.
    - A seventh is named only if it beats the plain triad by `SEVENTH_MARGIN` (0.02).
    - Romans carry the suffix (`Imaj7`, `vi7`, `V7`); the blueprint keeps it.
    - A bar that is silent or only a release tail (under 35% of the median bar energy) is skipped.
    - A chord counts at a strength of 0.6 or more (played chords now measure 0.97–1.0).
  - Beat-level change detection for the bar line compares root and quality, so a seventh flickering in and out is not a change.
