...
| **V3 Queue** | BUILT (cancelling a waiting item added in Session 016) | Up to 5 files per batch, converted one after another with per-item progress. The server also holds a lock around Seed-VC, so even separate requests never run two conversions at once (test with a fake provider: max concurrency 1) |
